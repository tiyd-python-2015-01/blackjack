...
def test_dealer_has_21():
    ...
    assert game1.dealer_has_blackjack() == True
    dealer2 = Dealer(Hand([Card(10, Diamonds), Card('A', Spades)]))
    game2 = Game(dealer2, player, shoe)
    assert game2.dealer_has_blackjack() == True
    dealer3 = Dealer(Hand([Card(10, Diamonds), Card('J', Spades)]))
    game3 = Game(dealer3, player, shoe)
    assert game3.dealer_has_blackjack() == False

def test_player_has_21():
    player1 = Player("John", hand = Hand([Card('J', Diamonds), Card('A', Spades)]))
    game1 = Game(dealer, player1, shoe)
    assert game1.player_has_blackjack() == True
    player2 = Player("Jill", hand = Hand([Card(10, Diamonds), Card('A', Spades)]))
    game2 = Game(dealer, player2, shoe)
    assert game2.player_has_blackjack() == True
    player3 = Player("Sam", hand = Hand([Card(10, Diamonds), Card('J', Spades)]))
    game3 = Game(dealer, player3, shoe)
    assert game3.player_has_blackjack() == False
